...
class Allocation:
    def __init__(self, proj,pers,max_fraction=1):        
        ...
        if proj.personDaysNeeded < self.arbitrary_cap_on_sharing_projects:
            ...
            can_be_allocated = min(int(proj.personDaysNeeded*max_fraction+1),self.project.daysStillNeeded)
        
        # the person will do as much of it as they have available
        if self.person.time_still_left > can_be_allocated: 
            ...
            self.person.changeTimeLeft(-1*can_be_allocated)
        else: #the person can do what they can
            self.time = self.person.time_still_left
            self.project.daysStillNeeded -= self.person.time_still_left
            self.person.allocated = True
            self.person.time_still_left = 0
